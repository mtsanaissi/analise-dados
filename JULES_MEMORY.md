...
*   **Start with the Environment**: My first step in a new task session must be to run pip install -r requirements.txt to ensure all dependencies are present. I should then run the test suite with pytest to confirm the environment is stable before making any code changes.
